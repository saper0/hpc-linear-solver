...
int main(){
	...
	//Create Log-Files
	ofstream file_log;
	file_log.open("performance_lu.txt");
	...
	while(n <= limit){
		...
		//Choose biggest column sum for LU-A => 1 Norm of LU-A
		res_norm = *max_element(res_colsum.begin(), res_colsum.end());
		//cout << res_norm << " " << A_norm << endl;
...
/**
* Calculates the LU Factorization of a given square matrix A stored in
...
solution LUfact(vector<vector<double>> A, int n){
	...
	//Unblocked LU-Factorization Algorithm implemented for a Matrix stored in row major order
	for(int k = 0; k < n; k++){
		if(A[k][k] == 0){
			s.error = 1;
			return s;
